...
while True:
    ...
    # Show only the S channel of the image
    frame_S = frame_HSV[:,:,1]
    frame_S = cv.bitwise_and(frame_S, frame_S, mask=frame_threshold)
    cv.imshow("S Channel", frame_S)
    # Show only the H channel of the image
    frame_H = frame_HSV[:,:,0]
    cv.imshow("H Channel", frame_H)
    ## [show]
