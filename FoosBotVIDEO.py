...
class FoosBot:
    def __init__(self, difficulty = 0):
        ...
        # video_path = 'C:/Users/juls6/Desktop/Classes/FOOSTABLE/Software/Sample_Video/Foosball_sample1.mp4'
        ...
        ##ROD CLASSES
        self.GRod = Rod(1, Control = False) ##initializes goalie rod
        self.DRod = Rod(2, Control = False) ##initializes defense rod
        self.MRod = Rod(3, Control = False) ##initializes midfield rod
        self.ARod = Rod(4, Control = False) ##initializes attack rod
        ...
        self.arduinoCommand = "" #the current command to send to the arduino
        # self.ser = serial.Serial(self.arduino_port,self.baud_rate, timeout = 1)
        self.serial_timer = time.time()
        self.waittime = .1 ##time to wait between position updates
    ...
    def UpdateGui(self):
        ...
        # Draw a yellow circle at the position of self.ball_pos_image
    
        cv.circle(window, self.ball_pos_image, 5, (0, 255, 255), -1)
        # Add text at the position of the circle
        text = f"Ball Position: {self.ball_pos_real}"
        cv.putText(window, text, self.ball_pos_image, cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    ...
    def UpdateFrame(self):
        ...
        self.current_frame = cv.resize(frame,(640, 480))
...
# 238]
    ...
    async def moveRods(self):
        ...
        # kicks the ball if it is in range
    ...
    def CalibrateRods(self):
        ...
        # self.ser.write(send.encode('utf-8'))
        # self.ser.read_until("RDY".encode('utf-8') + self.ENDCHAR.encode('utf-8'))
